...
namespace sgns
{
    ...
    void StreamWS(std::shared_ptr<boost::asio::io_context> ioc,
        ...
        ws->async_handshake(host, path, [ioc, ws, handle_read, parse, save](const boost::system::error_code& handshakeError) {
            if (!handshakeError) {
                //Request the file
                std::string request = "GET_FILE";
                ws->async_write(boost::asio::buffer(request), [ioc, ws, handle_read, parse, save](const boost::system::error_code& write_error, std::size_t bytes_transferred) {
                    if (!write_error) {
                        //Read until WSEOF
                        auto buffer = std::make_shared<boost::asio::streambuf>();
                        boost::asio::async_read_until(*ws, *buffer, "WSEOF", [ioc, ws, handle_read, parse, save, buffer](const boost::system::error_code& read_error, std::size_t bytes_transferred) {
                            if (!read_error)
                            {
    ...
    std::shared_ptr<void> WSLoader::LoadASync(std::string filename, bool parse, bool save, std::shared_ptr<boost::asio::io_context> ioc, CompletionCallback handle_read)
    {
        ...
        //Connect to server
        boost::asio::async_connect(ws->next_layer().next_layer(), results.begin(), results.end(), [ioc, ws, handle_read, parse, save, ws_host, ws_path](const boost::system::error_code& error, const auto&) {
            if (!error) {
                // Perform the SSL asynchronous handshake
                ws->next_layer().async_handshake(boost::asio::ssl::stream_base::client, [ioc, ws, handle_read, parse, save, ws_host, ws_path](const boost::system::error_code& handshakeError) {
                    if (!handshakeError) {
                        // Perform the WebSocket asynchronous handshake
                        StreamWS(ioc,ws,handle_read,parse,save,ws_host,ws_path);
                    }
                    else {
